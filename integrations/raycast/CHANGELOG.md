...
- Detail panel that leads with a plain-English conclusion and the evidence behind it, plus the launcher chain (rendered launcher-first, ending at "this process"), process state, subtree CPU (a headless browser burns CPU in child processes while its main process reads ~0%), memory, and uptime
