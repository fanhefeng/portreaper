...
已知检出盲区的案例存档。每条记录一个**真实发生过、当前版本确实漏掉**的场景：
现场数据、逐层追踪到具体放行的那一行、为什么它不是一个"顺手修掉"的 bug、
以及动手时必须同时满足的约束。
...
实现要点：
- macOS 数据源现成 —— `macos.rs` 已经在跑 `lsof`，把 `-sTCP:LISTEN` 之外的
  ESTABLISHED 行一并收下即可（对**同一 PID 的同一端口**统计对端连接数），
  不需要新增子进程调用。注意现有 lsof 调用带 `-sTCP:LISTEN` 过滤，要么放宽
  该过滤后自行分流，要么单独再取一次；权衡时记得 lsof 是本项目最贵的一次调用。
