...
待人工完成（本机无 Raycast，无法代劳）：

- [x] **`author` 字段** = Raycast 账号 handle `fhf1121`（不是 GitHub 用户名）——
      2026-08 已核对确认。
- [ ] **真机 QA**：在 Raycast 里把扩展**当成真用户用一遍**，确认代码写对了 ≠ 用起来是对的。
      `npm run dev` 后逐项走：

      | 场景 | 看什么 |
      |---|---|
      | 首次运行（支持目录里没有 CLI） | 下载进度是否有反馈；SHA-256 校验通过后能否直接进列表 |
      | 正常扫描 | 行是否按嫌疑度排序；置信度分档、端口、「no port」徽标显示是否正确 |
      | 展开详情 | 启动链、子树 CPU、内存、运行时长是否都有值且合理 |
      | kill 确认 | 弹窗是否出现、文案是否说清杀的是哪个进程；确认后行是否消失 |
      | 星标双向同步 | 在 Raycast 加星 → 桌面版 2 秒内是否出现 ★（反向再试一次） |
      | 搜索过滤 | 输入端口号 / 进程名能否正确筛选 |
      | 断网 | 拔网线后首次运行，是否渲染引导页而不是白屏或崩溃 |

      这几条**都不是 tsc / ray build 能替你验的** —— 它们只保证代码能编译、清单合规，
