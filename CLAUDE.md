...
- Whitelist (`is_whitelisted`) still lists the row, forces `is_zombie_suspect=false`, excluded from sweep/tray. Key (`scanner::mod::whitelist_key`, mirrored by `src/model.ts whitelistKey`): `exe_path` **only when it contains a path separator** — a PATH-resolved bare interpreter name (`ps -o comm=` returns just `node` for `node app.js`/shebang shims) would collapse the key across every same-named listener and whitelist them all, so bare names fall back to the full command line (then lsof `command`). Keep frontend and `scan()` byte-for-byte identical.
